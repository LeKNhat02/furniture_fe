...
import 'package:furniture_fe/models/filter_model.dart';
import 'package:furniture_fe/models/history_search_model.dart';
import 'package:furniture_fe/provider/category_provider.dart';
import 'package:furniture_fe/provider/filter_provider.dart';
import 'package:furniture_fe/screens/product_detail.dart';
...
class _SearchState extends State<Search> {
  ...
  Widget build(BuildContext context) {
    ...
    // filterProvider.loadFilters();
    filterList = [];

    listCart = cartProvider.cartItems;
    listFavorite = favoriteProvider.favorites;
    listHis = [];
    autoCompleteText = getAutoCompleteInput(
        productProvider.getListProduct, categoryProvider.categories);
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: Wrap(
                        ...
                        runSpacing: 20,
                        direction: Axis.horizontal,
                        children: categoryProvider.categories.map((e) {
                          return GestureDetector(
                            onTap: () {
                              setState(() {
                                page = 3;
  ...
  Widget getRawAutocompleteTextField() {
    //print(getAutoCompleteInput(productProvider.getListProduct, categoryProvider.categories).length);
  ...
  Widget getDropDownButtonCategory() {
    if (categoryProvider.categories.isNotEmpty) {
      ...
      for (var element in categoryProvider.categories) {
  ...
  Widget getExpandItem(String text , List<String> newLIst) {
    if(text != 'No selected') {
      ...
      for (var element in categoryProvider.categories[index-1].itemList) {
...
List<Product> getProductByFilter(List<Product> productList, FilterModel filter, String query) {
  ...
  if(filter.category.isNotEmpty) {
    String id = "";
    for(var data in categoryProvider.categories) {
